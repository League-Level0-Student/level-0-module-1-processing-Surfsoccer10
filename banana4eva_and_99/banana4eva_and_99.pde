for(int x= 99; x>1; x--){
  if(x==1){
   println ("" +x+ " bottle of beer on the wall," +x+ " bottle of beer Take one down and pass it around, no more bottles of beer on the wall.");
  }
  println("" +x+ " bottles of beer on the wall," +x+ " bottles of beer Take one down and pass it around," +(x-1)+" bottles of beer on the wall.");
  println(x);
  
}

println("1 bottle of beer on the wall, 1 bottle of beer. Take one down and pass it around, no more bottles of beer on the wall.");
println("No more bottles of beer on the wall, no more bottles of beer. Go to the store and buy some more, 99 bottles of beer on the wall.");
